...
import {
  ...
  CheckCircle, Mail, Phone, Globe, MapPin, Hash
} from 'lucide-react';
...
useEffect(() => {
  const fetchProfileData = async () => {
    try {
      ...
      if (customer.switchIps) {
        if (typeof customer.switchIps === "string") {
          try {
            ips = JSON.parse(customer.switchIps);
          } catch {
            console.warn("Invalid JSON format in switchIps");
            ips = [];
          }
        } else {
          // already array/object
          ips = customer.switchIps;
        }
      }
  ...
  const updateIpsInBackend = async (ips) => {
    try {
      await axiosInstance.put(`api/switchIps/${customerDetails.id}`, { switchIps: ips });
